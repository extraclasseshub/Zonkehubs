/*
  # Fix availability saving functionality

  1. Database Schema Updates
    - Ensure availability column exists with proper jsonb type
    - Ensure current_status column exists with proper constraints
    - Add proper indexes for performance

  2. Validation Functions
    - Create availability validation function
...
BEGIN
  ...
  IF EXISTS (
    ...
    -- Policy exists, drop it first
    DROP POLICY IF EXISTS "Providers can update own profile" ON service_providers;
  END IF;
  
  -- Create comprehensive update policy with correct auth.uid() function
