/*
  ...
  2. Data Validation
    - Robust availability schedule validation
    - Social media links validation
    - Website URL validation
    - Experience and certification validation

  3. Security & Performance
    - Proper row-level security policies
    - Optimized indexes for all new fields
...
BEGIN
  ...
  -- Availability column with proper type
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    ...
    RAISE NOTICE 'Added availability column with jsonb type';
  END IF;
...
DROP FUNCTION IF EXISTS validate_availability_trigger();
DROP FUNCTION IF EXISTS validate_provider_fields_trigger();
DROP FUNCTION IF EXISTS validate_availability(jsonb);
DROP FUNCTION IF EXISTS validate_social_media(jsonb);
DROP FUNCTION IF EXISTS validate_website_url(text);
...
BEGIN
  ...
  LOOP
    ...
    -- Basic URL/handle validation
    IF NOT (url ~* '^https?://.*' OR url ~* '^[a-zA-Z0-9@._+-]+$' OR url ~* '^\+[0-9]+$') THEN
      RETURN false;
    END IF;
  END LOOP;
...
BEGIN
  ...
  IF url ~* '^https?://[a-zA-Z0-9][a-zA-Z0-9.-]*[a-zA-Z0-9].*$' OR 
     url ~* '^[a-zA-Z0-9][a-zA-Z0-9.-]*[a-zA-Z0-9]\.[a-zA-Z]{2,}.*$' THEN
    RETURN true;
  END IF;
...
BEGIN
  ...
  IF NEW.availability IS NOT NULL THEN
    IF NOT validate_availability(NEW.availability) THEN
      RAISE EXCEPTION 'Invalid availability schedule format. Please check your time entries and ensure end times are after start times.';
    END IF;
  END IF;
  ...
  IF NEW.website IS NOT NULL AND NEW.website != '' THEN
    IF NOT validate_website_url(NEW.website) THEN
      RAISE EXCEPTION 'Invalid website URL format. Please enter a valid URL (e.g., https://example.com).';
    END IF;
  END IF;
...
DECLARE
  ...
  test_social jsonb := '{"facebook": "https://facebook.com/test", "instagram": "@testuser"}';
BEGIN
  -- Test availability validation
  IF validate_availability(test_availability) THEN
    RAISE NOTICE 'Availability validation test: PASSED';
  ELSE
    RAISE NOTICE 'Availability validation test: FAILED';
  END IF;
  
  -- Test social media validation
  IF validate_social_media(test_social) THEN
    RAISE NOTICE 'Social media validation test: PASSED';
  ELSE
    RAISE NOTICE 'Social media validation test: FAILED';
  END IF;
  
  -- Test website validation
  IF validate_website_url('https://example.com') THEN
    RAISE NOTICE 'Website validation test: PASSED';
  ELSE
    RAISE NOTICE 'Website validation test: FAILED';
  END IF;
END $$;
...
BEGIN
  ...
  IF column_count = 7 THEN
    RAISE NOTICE 'SUCCESS: All required columns exist (% out of 7)', column_count;
  ELSE
    RAISE NOTICE 'WARNING: Missing columns. Found % out of 7 required columns', column_count;
  END IF;
END $$;